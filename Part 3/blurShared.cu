...
__global__ void blurShared ( unsigned char * data,   unsigned char * out, std::size_t w, std::size_t h) {
  ...
  if( i < w && j < h ) {
    ...
    if( op1 > 0 && op1 < (blockDim.x - 1) && op2 > 0 && op2 < (blockDim.y - 1)){
      for (auto c = 0; c < 3; ++c){
			auto op4 = sharedExt[((op2 - 1) * op3 + op1 - 1) * 3 + c] +sharedExt[((op2 - 1) * op3 + op1 + 1) * 3 + c]
			+ sharedExt[( op2 * op3 + op1 - 1) * 3 + c] + sharedExt[( op2* op3 + op1 + 1) * 3 + c]
			+ sharedExt[((op2 + 1) * op3 + op1 - 1) * 3 + c] + sharedExt[((op2 + 1) * op3 + op1 + 1) * 3 + c]
			+ sharedExt[(( op2 - 1) * op3 + op1) * 3 + c] + sharedExt[( op2* op3 + op1) * 3 + c]
			+ sharedExt[(( op2 + 1) * op3 + op1) * 3 + c];
			out[(j * w + i) * 3 + c] = (op4 / 9);
		}
    }
  }
...
int main()
{
  ...
  blurShared<<< dim2, dim1, 3*dim1.x*dim1.y >>>( rgb_d, out, cols, rows );
  ...
  cv::imwrite( "BlurSharedOutput.jpg", img_out );
  cudaFree( rgb_d);
  cudaFree ( out);
  return 0;
}
