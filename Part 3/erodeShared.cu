  ...
  if( i < w && j < h ) {
    ...
    if( op1 > 0 && op1 < (blockDim.x - 1) && op2 > 0 && op2 < (blockDim.y - 1) )
    {
      for (auto c = 0; c < 3; ++c){
        ...
        auto op4 = umax(sharedExt[((op2 - 1) * op3 + op1 - 1) * 3 + c], sharedExt[((op2 - 1) * op3 + op1 + 1) * 3 + c]);
        op4 = umax(op4, sharedExt[(( op2 - 1) * op3 + op1) * 3 + c]);
        auto op6 = umax(sharedExt[( op2 * op3 + op1 - 1) * 3 + c],  sharedExt[( op2 * op3 + op1 + 1) * 3 + c]);
        op6 = umax(op6, sharedExt[( op2 * op3 + op1) * 3 + c]);
        auto op5 = umax(sharedExt[((op2 + 1) * op3 + op1 - 1) * 3 + c], sharedExt[((op2 + 1) * op3 + op1 + 1) * 3 + c]);
        op5 = umax(op5, sharedExt[((op2 + 1) * op3 + op1) * 3 + c]);

        auto res = umax (op4, op6);
        res = umax (res, op5);
...
int main()
{
  ...
  dim3 dim1( 32, 32 );
  dim3 op5( 3 * (( cols - 1) / (dim1.x-2) + 1) , ( rows - 1 ) / (dim1.y-2) + 1 );
  cudaEvent_t start, stop;
  cudaEventCreate( &start );
  cudaEventCreate( &stop );
  cudaEventRecord( start );
  erodeShared<<< op5, dim1, 3*dim1.x*dim1.y >>>( rgb_d, out, cols, rows );
  ...
  cv::imwrite( "ErodeSharedOutput.jpg", img_out );
  cudaFree( rgb_d);
  cudaFree ( out);
  return 0;
}
