...
__global__ void blur ( unsigned char * data,   unsigned char * out, std::size_t cols, std::size_t rows) {
  ...
  if ( op1 > 0 && op1 < (cols - 1) && op2 > 0 && op2 < (rows - 1)) {
		for (auto c = 0; c < 3; ++c){
		 ...
		 + data[( op2 * cols + op1 - 1) * 3 + c] + data[( op2 * cols + op1 + 1) * 3 + c]
		 + data[((op2 + 1) * cols + op1 - 1) * 3 + c] + data[((op2 + 1) * cols + op1 + 1) * 3 + c]
		 + data[(( op2 - 1) * cols + op1) * 3 + c] + data[( op2 * cols + op1) * 3 + c]
		 + data[(( op2 + 1) * cols + op1) * 3 + c];
...
int main()
{
  ...
  blur<<< dim2, dim1, 0, streams[ 0 ] >>>( rgb_d, out, cols, rows / 2 + 2);
  blur<<< dim2, dim1, 0, streams[ 1 ] >>>( rgb_d+size/2, out+size/2, cols, rows / 2 + 2);
  ...
  cv::imwrite( "BlurStreamOutput.jpg", img_out );
  cudaFree( rgb_d);
  cudaFree ( out);
  return 0;
}
