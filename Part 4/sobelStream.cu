...
__global__ void sobel ( unsigned char * data,   unsigned char * out, std::size_t cols, std::size_t rows) {
  ...
  if ( op1 > 0 && op1 < (cols - 1) && op2 > 0 && op2 < (rows - 1)) {
    ...
    for (auto c = 0; c < 3; ++c){
		  auto op3 =   data[((op2 - 1) * cols + op1 - 1) * 3 + c] - data[((op2 - 1) * cols + op1 + 1) * 3 + c]
		  + 2 * data[( op2 * cols + op1 - 1) * 3 + c] - 2 * data[( op2 * cols + op1 + 1) * 3 + c]
		  + data[((op2 + 1) * cols + op1 - 1) * 3 + c] - data[((op2 + 1) * cols + op1 + 1) * 3 + c];
		  auto op4 = data[((op2 - 1) * cols + op1 - 1) * 3 + c] - data[((op2 + 1) * cols + op1 - 1) * 3 + c]
		  + 2 * data[((op2 - 1) * cols + op1    ) * 3 + c] - 2 * data[((op2 + 1) * cols + op1    ) * 3 + c]
		  + data[((op2 - 1) * cols + op1 + 1) * 3 + c] - data[((op2 + 1) * cols + op1 + 1) * 3 + c];
		  auto res = op3*op3 + op4*op4;
		  res = res > 255*255 ? res = 255*255 : res;
		  out[(op2 * cols + op1) * 3 + c] = sqrt((float) res);
		}
	}
...
int main()
{
  ...
  sobel<<< dim2, dim1, 0, streams[ 0 ] >>>( rgb_d, out, cols, rows/4 + 2);
  sobel<<< dim2, dim1, 0, streams[ 1 ] >>>( rgb_d+size/4, out+size/4, cols, rows/4 + 2);
  sobel<<< dim2, dim1, 0, streams[ 2 ] >>>( rgb_d+size/2, out+size/4, cols, rows/4 + 2);
  sobel<<< dim2, dim1, 0, streams[ 3 ] >>>( rgb_d+3*size/4, out+3*size/4, cols, rows/4 );
  ...
  cv::imwrite( "SobelStreamOutput.jpg", img_out );
  cudaFree( rgb_d);
  cudaFree ( out);
  return 0;
}
