...
__global__ void blurShared ( unsigned char * data,   unsigned char * out, std::size_t w, std::size_t h) {
  ...
  if( op1 < w && op2 < h ) {
    ...
    {
      for (auto c = 0; c < 3; ++c){
        auto op6 = sharedExt[((op4 - 1) * op5 + op3 - 1) * 3 + c] + sharedExt[((op4 - 1) * op5 + op3 + 1) * 3 + c]
          + sharedExt[( op4 * op5 + op3 - 1) * 3 + c] + sharedExt[( op4 * op5 + op3 + 1) * 3 + c]
          + sharedExt[((op4 + 1) * op5 + op3 - 1) * 3 + c] + sharedExt[((op4 + 1) * op5 + op3 + 1) * 3 + c]
          + sharedExt[(( op4 - 1) * op5 + op3) * 3 + c] + sharedExt[( op4 * op5 + op3) * 3 + c]
          + sharedExt[(( op4 + 1) * op5 + op3) * 3 + c];
...
int main()
{
  ...
  blurShared<<< dim2, dim1, 3 * dim1.x * dim1.y, streams[ 0 ] >>>( rgb_d, out, cols, rows / 2 + 4);
  blurShared<<< dim2, dim1, 3 * dim1.x * dim1.y, streams[ 1 ] >>>( rgb_d+size/2, out+size/2, cols, rows / 2 + 4);
  ...
  cv::imwrite( "BlurSharedStreamOutput.jpg", img_out );
  cudaFree( rgb_d);
  cudaFree ( out);
