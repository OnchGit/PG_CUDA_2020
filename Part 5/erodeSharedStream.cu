...
__global__ void erodeShared ( unsigned char * data,   unsigned char * out, std::size_t w, std::size_t h) {
  ...
  if( op1 < w && op2 < h ) {
    ...
    {
      for (auto c = 0; c < 3; ++c){
        auto op6 = umax(sharedExt[((op4 - 1) * op5 + op3 - 1) * 3 + c], sharedExt[((op4 - 1) * op5 + op3 + 1) * 3 + c]);
        op6 = umax(op6, sharedExt[(( op4 - 1) * op5 + op3) * 3 + c]);
        auto op7 = umax(sharedExt[( op4 * op5 + op3 - 1) * 3 + c],  sharedExt[( op4 * op5 + op3 + 1) * 3 + c]);
        op7 = umax(op7, sharedExt[( op4 * op5 + op3) * 3 + c]);
        auto op8 = umax(sharedExt[((op4 + 1) * op5 + op3 - 1) * 3 + c], sharedExt[((op4 + 1) * op5 + op3 + 1) * 3 + c]);
        op8 = umax(op8, sharedExt[((op4 + 1) * op5 + op3) * 3 + c]);
        auto res = umax (op6, op7);
        res = umax (res, op8);
...
int main()
{
  ...
  erodeShared<<< dim2, dim1, 3 * dim1.x * dim1.y, streams[ 0 ] >>>( rgb_d, out, cols, rows / 2 + 2 );
  erodeShared<<< dim2, dim1, 3 * dim1.x * dim1.y, streams[ 1 ] >>>( rgb_d+size/2, out+size/2, cols, rows / 2);
  ...
  cv::imwrite( "ErodeSharedStreamOutput.jpg", img_out );
  cudaFree( rgb_d);
  cudaFree ( out);
